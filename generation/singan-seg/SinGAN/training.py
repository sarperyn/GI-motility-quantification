...
def train(opt,Gs,Zs,reals,NoiseAmp):
    ...
    while scale_num<opt.stop_scale+1:
        ...
        if (nfc_prev==opt.nfc):
            print('%s/%d/netG.pth' % (opt.out_, scale_num-1))
            ...
            if scale_num-2 >= 0:
                curr_dir = os.getcwd()
                print(curr_dir)
                #G_before = '%a/%s/%d/netG.pth' % (curr_dir, opt.out_, scale_num-2)
                #D_before = '%a/%s/%d/netD.pth' % (curr_dir, opt.out_, scale_num-2)
                # G_before = os.path.join(curr_dir, opt.out_, str(scale_num-2), 'netG.pth')
                # D_before = os.path.join(curr_dir, opt.out_, str(scale_num-2), 'netD.pth')
                # os.remove(G_before)
                # os.remove(D_before)
                shutil.rmtree(os.path.join(curr_dir, opt.out_, str(scale_num-2)))
                print(str(scale_num-2), str(scale_num-2))
                input()
